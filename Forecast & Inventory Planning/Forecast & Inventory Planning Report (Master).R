...
### May 2020 in S&OP forecast file

channel_start_col=30 #June col 
company_start_col=23 #June col
...
avai_BIUB <-avai_BIUB[,c(1,ncol(avai_BIUB))]
colnames(avai_BIUB)=c("Sku","avai_BIUB")
...
FG_SKU <-as.vector(unique(c(company_forecast[,1],as.vector(SKU_added$Sku))))
...
df_channel_consumption_combine$Note=substr(df_channel_consumption_combine$Note,1,nchar(df_channel_consumption_combine$Note)-1)
#### get rid of some one-off components being added into S&OP forecast
